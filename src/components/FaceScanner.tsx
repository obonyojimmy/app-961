...
export default function FaceScanner({
  ...
  const [validations, setValidations] = useState([
    ...
    { label: "turn-left", prompt: "Look to the left", passed: false },
    { label: "turn-right", prompt: "Look to the right", passed: false },
    { label: "turn-up", prompt: "Look up", passed: false },
    { label: "turn-down", prompt: "Look down", passed: false },
  ]);
  ...
  useEffect(() => {
    ...
    switch (nextValidation.label) {
      ...
      case "turn-down":
        if (Math.abs(headEulerAngleX) > -10) {
          markValidationPassed("turn-down");
          setLastValidationTime(now);
        }
        break;
  ...
  return (
    <View className=" items-center gap-4 ">
      ...
      <Text
        ...
        className="px-8 text-center text-xl font-semibold">
        {prompt}
      </Text>
      {details && (
        <Text color="error" className="px-8 text-center text-lg text-gray-500">
          {details}
        </Text>
      )}
    </View>
