...
export default function DashScreen() {
  ...
  return (
    <View className="flex-1 items-center justify-center gap-4 bg-white px-6">
      ...
      <Pressable className="rounded-xl bg-gray-800 px-6 py-3" onPress={() => handleLogout(false)}>
        <Text className="text-lg text-white">Logout</Text>
      </Pressable>
      <Pressable className="rounded-xl bg-gray-800 px-6 py-3" onPress={() => handleLogout(true)}>
        <Text className="text-lg text-white">Logout & Clear</Text>
      </Pressable>
    </View>
  );
}
