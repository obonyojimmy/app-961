...
import { Dimensions, NativeEventEmitter, NativeModules, Text, View } from "react-native";
...
export default function FaceScanScreen() {
  ...
  const getEmbedding = () => {
    VideoEffectModule.extractFaceEmbedding(
      (embeddingArray) => {
        ...
        // send to backend or match locally
      },
      (err) => {
  ...
  useEffect(() => {
    ...
    if (!spoofScores) return;

    setStatusMessage("");

    if (spoofScores[1] < 0.006) {
